...
export const listOffers = async (req, res) => {
  ...
  try {
    const [offers] = await pool.query(
      `SELECT j.id, j.position, j.sector, j.salary, j.work_mode, j.work_time, j.tech_stack, j.created_at,
...
export const applyToOffer = async (req, res) => {
  ...
  try {
    ...
    const [[offer]] = await pool.query(
      `SELECT j.position, j.sector, j.salary, j.work_mode, j.work_time, j.tech_stack, u.name as company_name
    ...
    await sendMail({
      ...
      html: `<h1>¡Hola ${dev.name}!</h1><p>Gracias por aplicar a la oferta de ${offer.company_name}.</p><p><strong>Puesto:</strong> ${offer.position}</p><p><strong>Salario:</strong> ${offer.salary}</p>`
    });
    ...
    if (company) {
      await sendMail({
        ...
        html: `<h1>¡Hola ${company.name}!</h1><p>Un desarrollador ha aplicado a tu oferta: <strong>${offer.position}</strong>.</p><p>Revisa tu panel para más detalles.</p>`
      });
    }
