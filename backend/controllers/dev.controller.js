...
export const getDeveloperProfile = async (req, res) => {
  ...
  try {
    const [rows] = await pool.query(
      `SELECT u.id, u.name, u.email, d.description, d.years_experience, d.location, d.tech_stack
       FROM users u
       JOIN developers d ON d.user_id = u.id
       WHERE u.id = ?`,
      [userId]
    );
    if (!rows.length) return res.status(404).json({ message: 'Developer profile not found' });
...
export const updateDeveloperProfile = async (req, res) => {
  ...
  try {
    await pool.query(
      `UPDATE developers SET description = ?, years_experience = ?, location = ?, tech_stack = ?
       WHERE user_id = ?`,
